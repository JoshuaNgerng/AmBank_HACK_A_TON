...
def identify_company(data: list[Source], db: Session) -> Company:
...
def process_pdf(self, report_id: int):
    with get_db_session() as db:
        ...
        company = identify_company(report.report_sources, db)
...
def analysis_ocr_result(self, report_id: int):
    with get_db_session() as db:
        ...
        try:
            ...
            check = extract_statement_from_sources(report, report.company)
            if not check['status']:
                raise ProgressException(
            ...
            check = extract_text_from_sources(report, report.company)
            if not check['status']:
                raise ProgressException(
...
def rerun_analysis_ocr_result(self, task_id: int):
    ...
    with get_db_session() as db:
        ...
        if not report.company:
            report.company = identify_company(report.report_sources, db)
        try:
            ...
            for state in ProgressState:
                if progress == state or not progress:
                    ...
                    check = func(report, report.company, task.immediatory_state, index)
                    if not check['status']:
                        raise ProgressException(
                            message=msg,
                            data=check['data'],
